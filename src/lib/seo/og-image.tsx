...
import { getHeadingNowFont, HEADING_NOW_FONT_NAME } from "./og-fonts";

export type OgImageVariant = "site" | "page" | "project";

export type PortfolioOgImageOptions = {
	subtitle?: string;
	title: string;
	variant: OgImageVariant;
};

export const PORTFOLIO_SOCIAL_IMAGE_SIZE = {
	width: 1200,
	height: 630,
} as const;

export const PORTFOLIO_SOCIAL_IMAGE_CONTENT_TYPE = "image/png";

function SabramanLogo({
	opacity = 1,
	size = 220,
}: {
	opacity?: number;
	...
	return (
		<svg
			aria-label="Sabraman logo"
			fill="none"
			role="img"
			style={{ display: "flex", height: size, opacity, width: size }}
...
function getAccentColor(variant: OgImageVariant) {
	switch (variant) {
		case "project":
			return "#ff197c";
		case "page":
...
function getTitleSize(title: string, variant: OgImageVariant) {
	if (variant === "site") {
		return 42;
	}
...
export async function createPortfolioOgImageResponse({
	subtitle,
	title,
	variant,
}: PortfolioOgImageOptions) {
	const accentColor = getAccentColor(variant);
	const fontData = await getHeadingNowFont();
		...
		return new ImageResponse(
			<div
				style={{
					alignItems: "center",
					background:
						"radial-gradient(circle at 20% 20%, rgba(139, 163, 255, 0.12) 0%, transparent 28%), radial-gradient(circle at 80% 80%, rgba(255, 25, 124, 0.08) 0%, transparent 30%), #000000",
					color: "#ffffff",
					display: "flex",
			...
			>
				<div
					style={{
						background:
							"linear-gradient(180deg, rgba(255,255,255,0.035) 0%, transparent 24%, transparent 76%, rgba(255,255,255,0.02) 100%)",
						inset: 0,
						position: "absolute",
					}}
				/>
				...
				>
					<SabramanLogo opacity={0.98} size={272} />
					<div
						style={{
							color: "rgba(255,255,255,0.9)",
							display: "block",
							fontFamily: HEADING_NOW_FONT_NAME,
							fontSize: 38,
			...
			</div>,
			{
				...
				fonts: [
					{ name: HEADING_NOW_FONT_NAME, data: fontData, style: "normal" },
				],
			},
		);
	}
	...
	return new ImageResponse(
		<div
			style={{
				background:
					"radial-gradient(circle at 18% 22%, rgba(139, 163, 255, 0.14) 0%, transparent 26%), radial-gradient(circle at 82% 78%, rgba(255, 25, 124, 0.12) 0%, transparent 28%), linear-gradient(135deg, #07080c 0%, #0b1018 54%, #07090d 100%)",
				color: "#ffffff",
				display: "flex",
				height: "100%",
				overflow: "hidden",
		...
		>
			...
			>
				...
				>
					...
					>
						<SabramanLogo opacity={0.94} size={72} />
					</div>
					<div
						style={{
					...
					>
						{variant === "project" ? "Case Study" : "Sabraman"}
					</div>
				</div>
				...
				>
					<div
						style={{
							display: "block",
							fontFamily: HEADING_NOW_FONT_NAME,
							fontSize: getTitleSize(title, variant),
							lineHeight: 0.92,
		...
		{
			...PORTFOLIO_SOCIAL_IMAGE_SIZE,
			fonts: [{ name: HEADING_NOW_FONT_NAME, data: fontData, style: "normal" }],
		},
	);
}
