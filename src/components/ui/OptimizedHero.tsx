...
function ParticleSystem() {
	...
	useFrame((state) => {
		...
		particles.forEach((particle, i) => {
			...
			// Update matrix directly for performance
			tempVector.set(particle.x, particle.y, particle.z);
	...
	return (
		...
		>
			...
			<meshBasicMaterial color="#7878fa" transparent opacity={0.6} />
		</instancedMesh>
	);
}
...
export function OptimizedHero({
	...
	const textY = useTransform(scrollYProgress, [0, 0.1], ["0%", "-30%"]);
	...
	return (
		...
		>
			...
			{/* Simple gradient overlay instead of complex animations */}
			<div className="absolute inset-0 bg-gradient-to-b from-background/90 via-background/60 to-background opacity-95" />
...
// Optimized logo component (removes performance-heavy effects)
export function OptimizedLogo({ displayName }: { displayName: string }) {
	return (
		<motion.div
			...
			className="mb-8"
		>
			{/* Your logo SVG without heavy animations */}
			<motion.svg
				width="200"
				height="200"
			...
			<h1
				className="font-extrabold text-[8vw] leading-none tracking-tight md:text-[6vw] lg:text-[5vw] xl:text-[4vw]"
				style={{
