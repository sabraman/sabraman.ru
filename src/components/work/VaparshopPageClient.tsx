...
export default function VaparshopPage({
	...
	const heroParticles = useMemo(() => {
		...
		return Array.from({ length: 14 }, (_, index) => ({
			...
			left: `${(index * 17) % 100}%`,
			top: `${(index * 23) % 100}%`,
			x: (index % 2 === 0 ? 1 : -1) * (18 + (index % 4) * 7),
			y: (index % 3 === 0 ? -1 : 1) * (16 + (index % 5) * 6),
			duration: 12 + (index % 6) * 1.8,
			delay: (index % 5) * 0.35,
		}));
						...
						{[
							...
							>
								<div className="mb-4 flex h-20 w-20 items-center justify-center">
									<tech.icon className={`h-10 w-10 ${tech.iconClassName}`} />
								</div>
						...
						{projects.map((project, index) => (
							...
							>
								<div className="relative aspect-video overflow-hidden bg-primary/20">
									...
									{/* Hover overlay */}
									<div className="absolute inset-0 flex items-center justify-center bg-accent/80 p-8 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
