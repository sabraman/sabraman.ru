...
export interface LegacyPickerContainerProps {
...
export function LegacyPickerContainer({
	children,
	className,
	frameWidth = "100%",
	width = "100%",
	...
	return (
		<div
			data-legacy-picker-root=""
			className={cn(
				"relative isolate flex h-[216px] w-full max-w-[320px] flex-col items-center justify-center overflow-hidden rounded-xl bg-[#20212f] px-[12px] py-[10px]",
				className,
			)}
			style={frameWidth !== "100%" ? { width: frameWidth } : undefined}
		>
			<div
				data-legacy-picker-content=""
			...
			<div
				className="pointer-events-none absolute inset-0 z-[1] rounded-[inherit] mix-blend-plus-lighter"
				style={{
					backgroundImage:
...
export interface LegacyPickerColumnProps<T extends string | number> {
...
export function LegacyPickerColumn<T extends string | number>({
	align = "center",
	className,
	fontSize = 25,
	infinite = true,
	...
	width,
	zIndex,
}: LegacyPickerColumnProps<T>) {
	return (
		<div
			data-legacy-picker-column=""
			className={cn(
			...
			style={
				{
					width,
					zIndex,
					"--legacy-picker-offset-y": `-${PICKER_OFFSET_Y}px`,
					"--legacy-picker-column-padding-x": `${padX}px`,
		...
		>
			<WheelPickerPrimitive.WheelPickerWrapper className="flex h-full w-full items-center justify-center border-0 bg-transparent shadow-none *:data-rwp:last:*:data-rwp-highlight-wrapper:rounded-none *:data-rwp:first:*:data-rwp-highlight-wrapper:rounded-none">
				...
				/>
				<style>{`
          [data-legacy-picker-column] [data-rwp] {
            transform: translateY(var(--legacy-picker-offset-y));
          }

          [data-legacy-picker-column] [data-rwp-item] {
            padding-left: var(--legacy-picker-column-padding-x);
            padding-right: var(--legacy-picker-column-padding-x);
          }
        `}</style>
